...
def mnistnet(images, num_classes=10, is_training=False,
  ...
  """
  end_points = {}
  with tf.variable_scope(scope, 'MnistNet', [images]):
    with tf.variable_scope('layer1'):
      end_point='conv1'
      net = slim.conv2d(images, 64, [5, 5], scope=end_point)
      end_point = 'conv2'
      net = slim.conv2d(net, 64, [5, 5], scope=end_point)
      end_point = 'pool3'
      net = slim.max_pool2d(net, [2, 2], scope=end_point)
      end_points['way1'] = net
    with tf.variable_scope('layer2'):
      end_point = 'conv1'
      net = slim.conv2d(net, 64, [1, 3], scope=end_point)
      end_point = 'conv2'
      net = slim.conv2d(net, 64, [3, 1], scope=end_point)
      end_point = 'conv3'
      net = slim.conv2d(net, 32, [5, 5], scope=end_point)
      end_point = 'pool4'
      net = slim.max_pool2d(net, [2, 2], scope=end_point)
      end_points['way2'] = net
